<?php
/*
 Plugin Name: Grezpay Gateway for WooCommerce
 Plugin URI: http://www.grezpay.com/
 Description: GrezPay PG WooCommerce integration
 Version: 1.0
 Author: Grezpay <support@grezpay.com>
 Author URI: http://www.grezpay.com
 */

 add_action('plugins_loaded', 'woocommerce_grezpay_init', 0);

/**
 * [woocommerce_grezpay_init description]
 * @return [type] [description]
 */
function woocommerce_grezpay_init()
{
    if (! class_exists('WC_Payment_Gateway')) {
        return;
    }
    class WC_Gateway_GrezPay extends WC_Payment_Gateway
    {
        public function __construct()
        {
            global $woocommerce;
            $this->id           = 'grezpay';
            $this->method_title = __('GrezPay Payment Gateway', 'woocommerce');
            $this->icon         = apply_filters('woocommerce_grezpay_icon', plugins_url().'/wc-grezpay/images/logo.png');
            ...
            // $this->pg_request_url       = 'https://merchant.grezpay.com/crm/jsp/paymentrequest';
            $this->pg_request_url       = 'https://uat.grezpay.com/crm/jsp/paymentrequest';

            // Actions
            add_action('init', array( $this, 'check_grezpay_response' ));
            add_action('woocommerce_api_wc_gateway_grezpay', array( $this, 'check_grezpay_response' ));
            add_action('woocommerce_receipt_grezpay', array(&$this, 'receipt_page'));
        ...
        {
            echo '<h3>GrezPay Payment Gateway</h3>';
            echo '<p>GrezPay Payment Gateway is a payment product which makes it extremely safe and easy to pay online.</p>';
        ...
        public function process_payment($order_id)
        {
            ...
            $this->return_url = trailingslashit(home_url()).'?wc-api=WC_Gateway_GrezPay&order_id='.$order->id;
            $this->init_settings();
        ...
        public function check_grezpay_response()
        {
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type'    => 'checkbox', 
                    'label'   => __( 'Enable GrezPay Gateway', 'woocommerce' ), 
                    'default' => 'yes'
                ), 
                'mode' => array(
                ...
                'title' => array(
                    ...
                    'default'     => __('GrezPay Gateway', 'woocommerce'),
                    'desc_tip'    => true,
                ),
                'description' => array(
    ...
    function add_grezpay_gateway($methods)
    {
        $methods[] = 'WC_Gateway_GrezPay';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_grezpay_gateway');
}
